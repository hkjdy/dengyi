...
* 	(defined in a properties file in application classpath) in Spring run-time Environment, 
* 	providing flexibility to have different values in different application environments.
* Method sessionFactory() is creating a LocalSessionFactoryBean, 
* 	which exactly mirrors the XML based configuration : 
* 	We need a dataSource and hibernate properties (same as hibernate.properties). 
* 	Thanks to @PropertySource, we can externalize the real values in a .properties file, 
* 	and use Spring¡¯s Environment to fetch the value corresponding to an item. 
...
@ComponentScan({ "cn.hkjdy.game.configuration" })
...
public class HibernateConfiguration {
    ...
    public LocalSessionFactoryBean sessionFactory() {
        ...
        sessionFactory.setPackagesToScan(new String[] { "cn.hkjdy.game.model" });
